define(['dart_sdk', 'packages/path/path'], function(dart_sdk, packages__path__path) {
  ...
  const UnparsedFrame_uri = dart.privateName(unparsed_frame, "UnparsedFrame.uri");
  const UnparsedFrame_line = dart.privateName(unparsed_frame, "UnparsedFrame.line");
  const UnparsedFrame_column = dart.privateName(unparsed_frame, "UnparsedFrame.column");
  const UnparsedFrame_isCore = dart.privateName(unparsed_frame, "UnparsedFrame.isCore");
  const UnparsedFrame_library = dart.privateName(unparsed_frame, "UnparsedFrame.library");
  const UnparsedFrame_package = dart.privateName(unparsed_frame, "UnparsedFrame.package");
  const UnparsedFrame_location = dart.privateName(unparsed_frame, "UnparsedFrame.location");
  const UnparsedFrame_member = dart.privateName(unparsed_frame, "UnparsedFrame.member");
  ...
  const uri = UnparsedFrame_uri;
  const line = UnparsedFrame_line;
  const column = UnparsedFrame_column;
  const isCore = UnparsedFrame_isCore;
  const library = UnparsedFrame_library;
  const package$ = UnparsedFrame_package;
  const location = UnparsedFrame_location;
  const member$ = UnparsedFrame_member;
  ...
  const VMTrace_frames = dart.privateName(vm_trace, "VMTrace.frames");
  ...
  const Frame_uri = dart.privateName(frame$, "Frame.uri");
  const Frame_line = dart.privateName(frame$, "Frame.line");
  const Frame_column = dart.privateName(frame$, "Frame.column");
  const Frame_member = dart.privateName(frame$, "Frame.member");
  ...
  const uri$ = Frame_uri;
  const line$ = Frame_line;
  const column$ = Frame_column;
  const member$0 = Frame_member;
  ...
  stack_zone_specification.StackZoneSpecification = class StackZoneSpecification extends core.Object {
    ...
    [_run](T, f, node) {
      ...
      try {
        ...
        t3 = this[_chains];
        t4 = stackTrace;
        t3._get(t4) == null ? t3._set(t4, node) : null;
        dart.rethrow(e$);
      } finally {
  ...
  let C0;
  const Chain_traces = dart.privateName(chain$, "Chain.traces");
  ...
  const Trace_frames = dart.privateName(trace$, "Trace.frames");
  const Trace_original = dart.privateName(trace$, "Trace.original");
  ...
  const frames$0 = Trace_frames;
  const original$ = Trace_original;
